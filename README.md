...
Pennsylvania. Built with Next.js (App Router), TypeScript, and Tailwind CSS v4.
...
build treats most of them as "not yet set" and hides the related UI (e.g. the
Facebook icon in the footer won't show until a real URL is added).
...
| Google rating & review count | `src/lib/site-config.ts` → `google`              | Not yet surfaced anywhere in the UI — add a display for it once both are set.                                                                                                                                                                                                                                                                  |
...
| Service area towns           | `src/lib/locations.ts`                           | Populated with 8 real Central PA towns as a starting point — confirm this list against the towns actually serviced, and avoid low-quality doorway pages for towns you don't serve.                                                                                                                                                             |
...
src/
  ...
  components/           UI components, organized flat with a ui/ subfolder
