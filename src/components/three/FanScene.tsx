    ...
    <group ref={group}>
      {blades.map((angle, i) => (
        ...
        >
          ...
          <meshStandardMaterial color="#1d4478" metalness={0.35} roughness={0.45} />
        </mesh>
      ))}
      <mesh rotation={[Math.PI / 2, 0, 0]}>
        <cylinderGeometry args={[0.32, 0.32, 0.14, 24]} />
        <meshStandardMaterial color="#de7c17" metalness={0.4} roughness={0.3} />
      </mesh>
    </group>
  );
}
    ...
    <group>
      {Array.from({ length: 10 }, (_, i) => (
        <mesh key={i} position={[0, 0, -0.06]} rotation={[0, 0, (i / 10) * Math.PI * 2]}>
          ...
          <meshStandardMaterial color="#0f2547" metalness={0.5} roughness={0.4} />
        </mesh>
      ))}
...
function Scene({ reduceMotion }: { reduceMotion: boolean }) {
  return (
    <>
      ...
      <directionalLight position={[-3, -2, 2]} intensity={0.35} color="#8fb9da" />
